...
for (const file of downloads.files) {
    console.log(`Local: ${file.local_url}`);
    console.log(`Public: ${file.public_url}`);
}
```

- **Returns**: `Promise<{ files: { local_url: string, public_url: string, status: 'pending'|'done'|'failed', error?: string }[] }>`
