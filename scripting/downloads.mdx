...
// 3. Wait for the download to finish and get the URL
const fileUrl = await runner.waitFor(async () => {
...
// 4. Use standard fetch() to read the file, whether it's OPFS or S3!
// Because we use standard web APIs, this correctly streams large files.
const res = await fetch(fileUrl);
const textData = await res.text();
console.log("File content length:", textData.length);

// 5. (Optional) Clear the queue if you are downloading multiple files sequentially
